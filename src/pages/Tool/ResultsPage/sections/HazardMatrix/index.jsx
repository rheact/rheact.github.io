...
const Matrix = ({ results, className }) => {
    if(!results || !results.hazardMatrix) {
        return (<h2 className={"text-muted " + className}>No hazard matrix generated.</h2>);
    }
    ...
    return (
        <div className={className}>
            ...
            <div className="d-flex flex-column">
                <span>The following links can guide the PPE selection process:</span>
                {links.map(link => <a key={link} href={link}>{link}</a>)}
            </div>

            <div className="text-muted w-100">Generated by parsing the H-phrases from the SDS and classifying the system components based on the handling hazards.</div>
        </div>
    )
}
